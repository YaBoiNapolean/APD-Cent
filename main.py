...
# Visual Identity - GSP Specific Hex Codes
...
# Role IDs for Permissions and Strikes
ROLES = {
...
def format_time_ago(ts_string):
    """
    Calculates relative time for NCIC reports.
    Converts 'Month Day, Year at HH:MM' back into 'X days/hours ago'.
    """
    try:
        past = datetime.strptime(ts_string, '%B %d, %Y at %H:%M')
        # We strip tzinfo for comparison with strptime result
...
async def init_db():
    """
    Initializes all 5 database tables.
    Includes persistent volume directory check for Railway.
    """
...
async def generate_unique_id():
    """
    Generates a unique GSP-XXXX ID.
    Checks all major record tables to prevent duplicates.
    """
    async with aiosqlite.connect(DATABASE) as db:
        while True:
            ...
            # Expanded check across all log tables
            query = """
                SELECT 1 FROM arrests WHERE id_code = ? 
                UNION SELECT 1 FROM citations WHERE id_code = ?
                UNION SELECT 1 FROM bolos WHERE id_code = ? 
                UNION SELECT 1 FROM warrants WHERE id_code = ?
            """
...
async def is_cmd_channel(itx: discord.Interaction):
    """
    Permission check to ensure commands are used in the proper channel.
    """
...
class StrikeConfirmView(ui.View):
    """
    Handles the 'Confirm Strike' button logic for Supervisors.
    """
    ...
    async def confirm_strike(self, itx: discord.Interaction, button: ui.Button):
        ...
        # Logic to determine next role
        target_role = s1
        if ub in self.trooper.roles:
            return await itx.response.send_message("⚠️ Already at 'Up For Ban'.", ephemeral=True)
        ...
        # Clean up database - mark infractions as used
        async with aiosqlite.connect(DATABASE) as db:
...
class ExpiryDropdown(ui.Select):
    """
    Selector for short-term log durations (BOLOs/Warrants).
    """
...
class InfractionExpiryDropdown(ui.Select):
    """
    Extended selector specifically for the infraction log command.
    """
    def __init__(self, callback_func):
        options = [
            discord.SelectOption(label="24 Hours", value="24"),
...
class ClearRecordConfirm(ui.View):
    """
    Logic for deleting a record ID.
    """
...
async def info(itx: discord.Interaction):
    """
    Displays bug report contact. Colored in GSP Custom Orange.
    """
...
async def trooper_performance(itx: discord.Interaction, trooper: discord.Member):
    """
    Retrieves full history from all database tables for a specific officer.
    """
    if not await is_cmd_channel(itx):
        return
...
async def search_record(itx: discord.Interaction, record_id: str):
    """
    Completely unconsolidated search logic for absolute clarity.
    """
    if not await is_cmd_channel(itx):
        return
...
@bot.tree.command(name='clear_record', description='Delete a record by ID')
async def clear_record(itx: discord.Interaction, record_id: str):
    """
    Prompts for deletion of a record based on ID.
    """
    if not await is_cmd_channel(itx):
        return
    rid = record_id.upper()
        ...
        for table in ["arrests", "citations", "bolos", "warrants"]:
            async with db.execute(f"SELECT officer_id FROM {table} WHERE id_code = ?", (rid,)) as c:
                ...
                if row:
                    ...
                    return await itx.response.send_message(f"🚨 Delete `{rid}`?", view=view, ephemeral=True)
...
async def infraction_log(itx: discord.Interaction, trooper: discord.Member, reason: str, punishment: str, proof: str = "None"):
    """
    Logs internal infractions with long-term expiry options.
    """
    if not await is_cmd_channel(itx):
        return
...
async def search_user(itx: discord.Interaction, suspect_name: str):
    """
    Provides a civilian history profile.
    """
    if not await is_cmd_channel(itx):
        return
    ...
    ncic_embed.description = f"{SEPARATOR}\n**Warrants:** {w_text}\n**BOLOs:** {b_text}\n**Last Arrest:** {arr_text}\n{SEPARATOR}"
...
async def bolo_log(itx: discord.Interaction, suspect: str, vehicle: str, reason: str, plate: str = "Unknown"):
    """
    Issues a BOLO for a set duration.
    """
    if not await is_cmd_channel(itx):
        return
...
async def warrant_log(itx: discord.Interaction, suspect: str, reason: str, risk: str = "Medium"):
    """
    Issues a warrant for a set duration.
    """
    if not await is_cmd_channel(itx):
        return
...
async def user_info(itx: discord.Interaction, trooper: discord.Member):
    """
    Displays technical Discord information for a trooper.
    """
    if not await is_cmd_channel(itx):
        return
    info_embed = discord.Embed(title=f"👤 PROFILE: {trooper.display_name}", color=GSP_CUSTOM_ORANGE)
    info_embed.description = f"{SEPARATOR}\n**ID:** `{trooper.id}`\n**Join Date:** {trooper.joined_at.strftime('%Y-%m-%d')}\n{SEPARATOR}"
    await itx.response.send_message(embed=info_embed)

@bot.event
async def on_ready():
    """
    Bot startup. Syncs commands and initializes database.
    """
    await init_db()
    await bot.tree.sync()
...
# --- BOT EXECUTION ---
# Ensure the DISCORD_TOKEN is set in your Railway Environment Variables.
